...
var rsaKey = {
  alg: "RS256",
  //private key - do not store in code
...
// in-memory data store in prod use database to store these info
var refreshTokens = {};
  ...
  if (req.body.grant_type == "authorization_code") {
    ...
    if (code) {
      ...
      if (code.request.client_id == clientId) {
        ...
        // save this to a database in production [for introspection piece - refresh token]
        accessTokens.push({
          access_token: access_token,
          client_id: clientId,
          scope: code.scope,
        });
        //refreshToken in scenario if client want access token using the refresh token
